...
function setScrollAnimation(element, order = 0) {
    let yMovement = 0;
...
function computeChance(entities, standard, base = 1000) {
    const totalPercentage = entities.reduce((acc, x) => acc + x.chance, 0);
    if (totalPercentage > base)
        throw Error(`Percentage exceeds 100%`);
